...
public class AuxipStatusControllerImpl implements StatusController {
	 ...
	 *         error message, if an invalid AUXIP Monitor identifier was passed
	 */
	@Override
	public ResponseEntity<RestInterfaceStatus> getRestInterfaceStatusByAuxipid(String auxipid, HttpHeaders httpHeaders) {
		if (logger.isTraceEnabled())
			logger.trace(">>> getRestInterfaceStatusByAuxipid({})", auxipid);

		if (!config.getAuxipId().equals(auxipid)) {
			return new ResponseEntity<>(http.errorHeaders(logger.log(ApiMonitorMessage.INVALID_AUXIP_ID, auxipid)),
