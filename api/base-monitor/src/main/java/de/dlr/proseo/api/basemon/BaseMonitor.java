...
public abstract class BaseMonitor extends Thread {
	...
	private Set<String> readTransferHistory() throws IOException {
		if (logger.isTraceEnabled())
			logger.trace(">>> readTransferHistory()");
		...
		logger.log(ApiMonitorMessage.HISTORY_ENTRIES_READ, transferHistory.size(), transferHistoryFile,
	...
	private void writeTransferHistory(String transferObjectIdentifier, Instant checkTimeStamp) throws IOException {
		if (logger.isTraceEnabled())
			logger.trace(">>> writeTransferHistory({})", transferObjectIdentifier);
	...
	private void truncateTransferHistory() throws IOException {
		if (logger.isTraceEnabled())
			logger.trace(">>> truncateTransferHistory()");
	...
	/**
	 * Check the given list of objects against the transfer history and return a new list containing only the objects not yet
	 * transferred (based on the object identifiers)
	 *
	...
	private List<TransferObject> filterTransferableObjects(List<TransferObject> transferableObjects) {
		if (logger.isTraceEnabled())
			logger.trace(">>> filterTransferableObjects(TransferObject[{}])",
		...
		// Apply filter
		if (logger.isTraceEnabled())
			logger.trace("... filtering against transfer history of size " + transferHistory.size());
		...
		if (logger.isTraceEnabled())
			logger.trace("... objects to transfer after filtering: " + objectsToTransfer.size());
	...
	synchronized private void recordTransfer(TransferObject transferredObject) throws IOException {
		if (logger.isTraceEnabled())
			logger.trace(">>> recordTransfer({})", null == transferredObject ? "null" : transferredObject.getIdentifier());
	...
	public void run(Integer maxCycles) {
		...
		while (null == maxCycles || i < maxCycles) {
			...
			if (logger.isTraceEnabled())
				logger.trace("... entering check loop # " + i);
			...
			try {
				if (logger.isTraceEnabled())
					logger.trace("... sleeping for {} seconds", checkInterval / 1000.0);
			...
			if (logger.isTraceEnabled())
				logger.trace("... leaving check loop # " + i);
			if (null != maxCycles)
				++i;
	...
	/**
	 * Run forever (as required by Thread superclass, at the same time convenience function for calling {@link #run(Integer)} with a
