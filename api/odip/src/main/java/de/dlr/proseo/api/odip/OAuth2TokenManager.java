...
import de.dlr.proseo.api.odip.rest.model.OAuth2Response;
...
public class OAuth2TokenManager {
	...
	// We need exactly 256 bits (32 bytes) of key length, so a shorter key will be filled with blanks, a longer key will be
	// truncated
	private static final byte[] TOKEN_SECRET = Arrays
		.copyOf((String.valueOf(Math.random()) + "                                ").getBytes(), 32);
	...
	/**
	 * Information about a user including cached password for authentication with prosEO backend services and authorities to
	 * determine access rights to order and workflow classes depending on their visibility
	 */
	...
	private String createToken(UserInfo userInfo) throws SecurityException {
		...
		JWTClaimsSet claims = new JWTClaimsSet.Builder().subject(userInfo.missionCode + "\\" + userInfo.username)
		...
		try {
			...
			throw new RuntimeException(logger.log(OdipMessage.MSG_EXCEPTION, e.getClass().getCanonicalName(), e.getMessage()));
		}
	 ...
	 *
	 * @param grantType type of grant requested (value must be set to "password" for "Resource Owner Password Credentials Grant"
	 *                  flow or to "client_credentials" for "Client Credentials Grant" flow as per RFC 6749; REQUIRED)
	 * @param username  the ODIP username (as per RFC 6749; REQUIRED for "Resource Owner Password Credentials Grant" flow, must not
	 *                  be set otherwise)
	 * @param password  the ODIP password (as per RFC 6749; REQUIRED for "Resource Owner Password Credentials Grant" flow, must not
	 *                  be set otherwise)
	...
	public OAuth2Response getToken(String grantType, String username, String password, HttpHeaders headers)
		...
		case GRANT_TYPE_PASSWORD:
			...
			if (2 != usernameParts.length) {
				throw new IllegalArgumentException(logger.log(OdipMessage.MSG_USERNAME_INVALID, username));
			}
			// User name and password must match information from Authorization header
			if (!userInfo.missionCode.equals(usernameParts[0]) || !userInfo.username.equals(usernameParts[1])
					|| !userInfo.password.equals(password)) {
				throw new IllegalArgumentException(logger.log(OdipMessage.MSG_CREDENTIAL_MISMATCH, username));
			}
			break;
		case GRANT_TYPE_CLIENT_CREDENTIALS:
			if (null != username || null != password) {
				throw new IllegalArgumentException(logger.log(OdipMessage.MSG_SUPERFLUOUS_PARAMETERS));
			}
			break;
		default:
			throw new UnsupportedOperationException(logger.log(OdipMessage.MSG_GRANT_TYPE_INVALID, grantType));
		}
		...
		logger.log(OdipMessage.MSG_TOKEN_CREATED, username);
	...
	public UserInfo getUserInfoFromToken(String token) throws SecurityException {
		if (logger.isTraceEnabled())
			logger.trace(">>> getUserInfoFromToken({})", token);
		...
		try {
			...
			throw new SecurityException(logger.log(OdipMessage.MSG_TOKEN_INVALID, token, MSG_TOKEN_NOT_PARSEABLE));
		}
		...
		try {
			if (!signedJWT.verify(verifier)) {
				throw new SecurityException(logger.log(OdipMessage.MSG_TOKEN_INVALID, token, MSG_TOKEN_VERIFICATION_FAILED));
			}
		} catch (IllegalStateException e) {
			throw new SecurityException(
					logger.log(OdipMessage.MSG_TOKEN_INVALID, token, MSG_TOKEN_STATE_INVALID + signedJWT.getState()));
		} catch (JOSEException e) {
			throw new SecurityException(logger.log(OdipMessage.MSG_TOKEN_INVALID, token, MSG_TOKEN_NOT_VERIFIABLE));
		}
		...
		try {
			...
			throw new SecurityException(logger.log(OdipMessage.MSG_TOKEN_INVALID, token, MSG_TOKEN_PAYLOAD_INVALID));
		}
		if (null == claimsSet.getSubject() || null == claimsSet.getExpirationTime()) {
			throw new SecurityException(logger.log(OdipMessage.MSG_TOKEN_INVALID, token, MSG_TOKEN_PAYLOAD_INCOMPLETE));
		}
		...
		if (null == userInfo) {
			throw new SecurityException(
					logger.log(OdipMessage.MSG_TOKEN_INVALID, token, MSG_USER_INVALID + claimsSet.getSubject()));
		}

		if ((new Date()).after(claimsSet.getExpirationTime())) {
			throw new SecurityException(logger.log(OdipMessage.MSG_TOKEN_EXPIRED, claimsSet.getExpirationTime()));
		}
