...
public class OAuth2TokenManager {
	 ...
	 * determine access rights to product classes depending on their visibility
	 */
	 ...
	 * @param username  the PRIP username (as per RFC 6749; REQUIRED for "Resource Owner Password Credentials Grant" flow, must not
	 *                  be set otherwise)
	 * @param password  the PRIP password (as per RFC 6749; REQUIRED for "Resource Owner Password Credentials Grant" flow, must not
	 *                  be set otherwise)
	...
	public UserInfo getUserInfoFromToken(String token) throws SecurityException {
		...
		try {
			...
			throw new SecurityException(
					logger.log(PripMessage.MSG_TOKEN_INVALID, token, MSG_TOKEN_STATE_INVALID + signedJWT.getState()));
		} catch (JOSEException e) {
		...
		if (null == userInfo) {
			throw new SecurityException(
					logger.log(PripMessage.MSG_TOKEN_INVALID, token, MSG_USER_INVALID + claimsSet.getSubject()));
		}
