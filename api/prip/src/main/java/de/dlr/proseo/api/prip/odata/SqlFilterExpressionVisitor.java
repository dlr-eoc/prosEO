...
public class SqlFilterExpressionVisitor implements ExpressionVisitor<String> {
	...
	private static final String[][] ODATA_TO_SQL_MAPPING = { { ProductEdmProvider.GENERIC_PROP_ID, "p.uuid" },
			...
			{ ProductEdmProvider.ET_PRODUCT_PROP_CONTENT_DATE + "/" + ProductEdmProvider.CT_TIMERANGE_PROP_START,
					"p.sensing_start_time" },
			{ ProductEdmProvider.ET_PRODUCT_PROP_CONTENT_DATE + "/" + ProductEdmProvider.CT_TIMERANGE_PROP_END,
					"p.sensing_stop_time" },
			...
			{ ProductEdmProvider.ET_PRODUCT_PROP_CHECKSUM + "/" + ProductEdmProvider.CT_CHECKSUM_PROP_CHECKSUM_DATE,
					"pf.checksum_time" },
			...
			{ CscAttributeName.INSTRUMENT_SHORT_NAME.getValue(), "TRUE" }, // Actually we cannot select by instrument short name,
																			// it's not in the data model yet
	...
	{
		for (String[] element : ODATA_TO_SQL_MAPPING) {
			oDataToSqlMap.put(element[0], element[1]);
		}
	}

	/**
	 * Get the applicable SQL command up to and including the 'WHERE' keyword (the remainder has been created by the visit*
	 * methods).
	 *
	 ...
	 * @return The SQL representation of the member expression
	 * @throws ExpressionVisitException  If an error occurs while visiting the expression
	 * @throws ODataApplicationException If an invalid property name or unsupported URI resource kind is encountered
	 */
	@Override
	public String visitMember(final Member member) throws ExpressionVisitException, ODataApplicationException {
		...
		if (UriResourceKind.primitiveProperty.equals(uriResource.getKind())) {
			...
			if (logger.isTraceEnabled())
				logger.trace("... found primitive property: " + propertyName);
			String mappedProperty = oDataToSqlMap.get(propertyName);
			if (logger.isTraceEnabled())
				logger.trace("... mapped primitive property: " + mappedProperty);
		...
		case complexProperty:
			...
			if (logger.isTraceEnabled())
				logger.trace("... found complex property: " + edmProperty.getName());
		...
		case navigationProperty:
			...
			if (logger.isTraceEnabled())
				logger.trace("... found navigation property: " + edmNavProperty.getName());
			propertyName.append(edmNavProperty.getName());
			break;
		default:
			if (logger.isTraceEnabled())
				logger.trace("... found unknown property of kind: " + uriResource.getKind());
			throw new ODataApplicationException(
					"Only primitive, complex, navigation and lambda variable URI resources are implemented in filter expressions",
		...
		while (uriResourceIter.hasNext()) {
			...
			propertyName.append('/');
			if (logger.isTraceEnabled())
				logger.trace("... next path property of kind: " + uriResource.getKind());
			...
			case lambdaAny:
				...
				String result = visitLambdaExpression(lambdaResource.getSegmentValue(), lambdaResource.getLambdaVariable(),
						lambdaResource.getExpression());
				if (logger.isTraceEnabled())
					logger.trace("... translation of lambda function 'any': " + result);
				if (logger.isTraceEnabled())
					logger.trace("<<< visitMember()");
				return result;
			default:
				throw new ODataApplicationException(
						"Only primitive, complex, navigation and lambda 'any' URI resources and arrays thereof are allowed as sub-paths in filter expressions",
						HttpStatusCode.NOT_IMPLEMENTED.getStatusCode(), Locale.ENGLISH);
			}

		}
		if (logger.isTraceEnabled())
			logger.trace("... derived property: " + propertyName);
		String mappedProperty = oDataToSqlMap.get(propertyName.toString());
		if (logger.isTraceEnabled())
			logger.trace("... mapped property: " + mappedProperty);
	 ...
	 * @return The SQL representation of the literal expression
	 * @throws ExpressionVisitException  If an error occurs while visiting the expression
	 * @throws ODataApplicationException If an invalid or unsupported literal type is encountered
	 */
	@Override
	public String visitLiteral(Literal literal) throws ExpressionVisitException, ODataApplicationException {
		...
		if (literal.getType() instanceof EdmString) {
			...
			throw new ODataApplicationException(
					"Only Edm.Boolean, Edm.Byte, Edm.SByte, Edm.Int16, Edm.Int32, Edm.Int64, Edm.String, Edm.DateTimeOffset, Edm.Decimal, Edm.Single, and Edm.Double literals are implemented",
					HttpStatusCode.NOT_IMPLEMENTED.getStatusCode(), Locale.ENGLISH);
		}
	 ...
	 * @return The SQL representation of the unary operator expression
	 * @throws ExpressionVisitException  If an error occurs while visiting the expression
	 * @throws ODataApplicationException If an invalid unary operator type is encountered
	 */
	@Override
	public String visitUnaryOperator(UnaryOperatorKind operator, String operand)
			throws ExpressionVisitException, ODataApplicationException {
		if (logger.isTraceEnabled())
			logger.trace(">>> visitUnaryOperator({}, {})", operator, operand);
		...
		if (logger.isTraceEnabled())
			logger.trace("<<< visitUnaryOperator()");
		return result;
	}
	 ...
	 * @throws ExpressionVisitException  If an error occurs while visiting the expression
	 * @throws ODataApplicationException If an invalid or unsupported binary operator type is encountered
	 */
	@Override
	public String visitBinaryOperator(BinaryOperatorKind operator, String left, String right)
		...
		// Binary Operators are split up into three different kinds. Up to the kind of the operator, it can be applied
		// to different types
	 ...
	 * @throws ODataApplicationException if the parameters are not applicable or insufficient for the requested method, or if an
	 *                                   unimplemented method was requested
	 */
	@Override
	public String visitMethodCall(MethodKind methodCall, List<String> parameters)
			throws ExpressionVisitException, ODataApplicationException {
		if (logger.isTraceEnabled())
			logger.trace(">>> visitMethodCall({}, {})", methodCall, parameters);
		...
		case CONTAINS:
			...
			if (parameters.get(0) instanceof String && parameters.get(1) instanceof String) {
				String valueParam1 = parameters.get(0);
				String valueParam2 = parameters.get(1);
	 ...
	 * @return The SQL representation of the type literal
	 * @throws ExpressionVisitException  If an error occurs while visiting the expression
	 * @throws ODataApplicationException If type literals are not implemented
	 */
	@Override
	public String visitTypeLiteral(EdmType type) throws ExpressionVisitException, ODataApplicationException {
		throw new ODataApplicationException("Type literals are not implemented", HttpStatusCode.NOT_IMPLEMENTED.getStatusCode(),
				Locale.ENGLISH);
	}
	 ...
	 * @return The SQL representation of the alias expression
	 * @throws ExpressionVisitException  If an error occurs while visiting the expression
	 * @throws ODataApplicationException If aliases are not implemented
	 */
	@Override
	public String visitAlias(String aliasName) throws ExpressionVisitException, ODataApplicationException {
		throw new ODataApplicationException("Aliases are not implemented", HttpStatusCode.NOT_IMPLEMENTED.getStatusCode(),
				Locale.ENGLISH);
	}
	 ...
	 *
	 * @param type       The EDM enum type
	 * @param enumValues The list of enum values
	 * @return The SQL representation of the enum expression
	 * @throws ExpressionVisitException  If an error occurs while visiting the expression
	 * @throws ODataApplicationException If enum conversion fails or an unsupported enum type is encountered
	 */
	@Override
	public String visitEnum(EdmEnumType type, List<String> enumValues) throws ExpressionVisitException, ODataApplicationException {
		if (logger.isTraceEnabled())
			logger.trace(">>> visitEnum({}, {})", type, enumValues);
	 ...
	 * @return The SQL representation of the lambda expression
	 * @throws ExpressionVisitException  If an error occurs while visiting the expression
	 * @throws ODataApplicationException If an invalid attribute name is found in the attribute lambda expression
	 */
	@Override
	public String visitLambdaExpression(String lambdaFunction, String lambdaVariable, Expression expression)
			throws ExpressionVisitException, ODataApplicationException {
		if (logger.isTraceEnabled())
			logger.trace(">>> visitLambdaExpression({}, {}, {})", lambdaFunction, lambdaVariable, expression);
		...
		if (logger.isTraceEnabled())
			logger.trace("... got attribute condition: " + attCondition);
		...
		if (null == mappedProperty) {
			...
			if (null == CscAttributeName.get(attCondition.getName())) {
				throw new ODataApplicationException(
						"Invalid attribute name '" + attCondition.getName() + "' found in Attribute lambda expression",
						HttpStatusCode.BAD_REQUEST.getStatusCode(), Locale.ENGLISH);
			}
			++paramCount;
			result = String.format(PARAMETER_WHERE_TEMPLATE, paramCount, attCondition.getName(), paramCount, attCondition.getOp(),
					attCondition.getValue());
		} else {
		...
		if (logger.isTraceEnabled())
			logger.trace("<<< visitLambdaExpression()");
		return result;
	}
	 ...
	 * @return The SQL representation of the lambda reference expression
	 * @throws ExpressionVisitException  If an error occurs while visiting the expression
	 * @throws ODataApplicationException If lambda references are not implemented
	 */
	@Override
	public String visitLambdaReference(String variableName) throws ExpressionVisitException, ODataApplicationException {
		throw new ODataApplicationException("Lamdba references are not implemented", HttpStatusCode.NOT_IMPLEMENTED.getStatusCode(),
				Locale.ENGLISH);
	}
	 ...
	 * @return The SQL representation of the binary operator expression
	 * @throws ExpressionVisitException  If an error occurs while visiting the expression
	 * @throws ODataApplicationException If comparing string to a string list or the binary operator is not implemented
	 */
	@Override
