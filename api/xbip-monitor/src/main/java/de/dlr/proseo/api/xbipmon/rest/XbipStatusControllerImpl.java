...
public class XbipStatusControllerImpl implements StatusController {
	 ...
	 *         error message, if an invalid XBIP Monitor identifier was passed
	 */
	@Override
	public ResponseEntity<RestInterfaceStatus> getRestInterfaceStatusByXbipid(String xbipid, HttpHeaders httpHeaders) {
		if (logger.isTraceEnabled())
			logger.trace(">>> getRestInterfaceStatusByXbipid({})", xbipid);

		if (!config.getXbipId().equals(xbipid)) {
			return new ResponseEntity<>(http.errorHeaders(logger.log(ApiMonitorMessage.INVALID_XBIP_ID, xbipid)),
