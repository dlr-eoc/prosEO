...
public class OrbitTimeFormatter {
	...
	private static Pattern dateTimePattern = 
			Pattern.compile("(?<year>\\d\\d\\d\\d)-(?<month>\\d\\d)-(?<day>\\d\\d)" + // date (groups 1-3)
	...
	private static final DateTimeFormatter orbitTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSSSSS")
	...
	/**
	 * Parse a date and time string in the format
	 * "yyyy-MM-dd'T'HH:mm:ss.SSSSSS[zZX]", whereby all of the following variants
	 * are allowed:
	 * <ul>
	 * <li>yyyy-MM-dd</li>
	 ...
	 * <li>any of the above plus a time zone in general, RFC 822 or ISO 8601
	 * format</li>
	 * </ul>
	 ...
	 * @throws DateTimeException if the given string cannot be parsed according to
	...
	public static Instant parseDateTime(String dateTime) throws DateTimeParseException {
		...
		if (null != m.group("zone")) {
			...
			if ("Z".equals(tzString.toUpperCase())) {
				...
				throw new DateTimeParseException(String.format(MSG_CANNOT_PARSE_DATE_TIME_STRING, dateTime, m.start("zone")),
						dateTime, m.start("zone"));
			}
		}
