...
PLATFORM=${PROSEO_PLATFORM}

function build_component {
  echo "Building $component"
  cd $component
  IMAGENAME=$(cat Dockerfile | grep FROM | awk '{gsub("localhost:5000/",""); split($0,a," "); print a[2]}')
  echo "Image name: $IMAGENAME"
  docker build -q -t $REGISTRY/$IMAGENAME --platform ${PLATFORM} .
  cd - >/dev/null
}
