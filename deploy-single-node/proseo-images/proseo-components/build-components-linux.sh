...
for component in proseo-*; do
    ...
    if [ -f "application-linux.yml.template" ]; then
        envsubst < application-linux.yml.template > application.yml
    else
        echo "Warning: No application-linux.yml.template found in ${component}. Skipping."
        # continue
    fi
