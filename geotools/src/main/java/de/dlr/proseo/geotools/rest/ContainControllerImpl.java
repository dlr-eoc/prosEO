...
/**
 * Controller to handle the contains request, which checks whether a specified
 * polygon is contained in selected or all regions from application.yml.
 *
...
public class ContainControllerImpl implements ContainController {
	...
	private static ProseoLogger logger = new ProseoLogger(ContainControllerImpl.class);
	...
	/**
	 * Checks whether an area, i.e. the Polygon defined by the array poly is
	 * contained in at least one of the provided regions. If none are given, all
	 * available region types will be considered.
	 *
	 * For longer arrays, using the containspoly() method with a RestPolygon is
	 ...
	 * @return HttpStatus 200 and true if the polygon is contained in one or more of
	 *         the specified regions (all known regions if none were specified),
	 *         false otherwise OR HttpStatus 400 and an error message if the input was
	...
	public ResponseEntity<Boolean> contains(String[] poly, String[] type) {
		...
		if (null != poly && poly.length == 2) {
			...
			// Check whether the point defined by latitude and longitude is contained
		...
		// If a geographical are was defined:
			...
			// Check whether the region defined by the RestPolygon is contained in at
			// least one of the specified regions
			try {
				Boolean contains = geotools.isPolyInside(polygon, type);
				return new ResponseEntity<>(contains, HttpStatus.OK);
			} catch (UnsupportedOperationException e) {
				return new ResponseEntity<>(http.errorHeaders(e.getMessage()), HttpStatus.INTERNAL_SERVER_ERROR);
			} catch (IllegalArgumentException e) {
	...
	/**
	 * Checks whether an area defined by a RestPolygin is contained in at least one
	 * of the provided regions. If none are given, all available region types will
	 * be considered.
	 *
	 ...
	 */
	@Override
	public ResponseEntity<Boolean> containspoly(String[] type, RestPolygon restPolygon) {
		...
		if (null != restPolygon && restPolygon.getPoints().size() == 1) {
			...
			// in at least one of the specified regions
			try {
				Boolean contains = geotools.isPointInside(restPolygon.getPoints().get(0).getLat(),
		...
		// If a geographical are was defined:
			...
			// least one of the specified regions
			try {
				Boolean contains = geotools.isPolyInside(restPolygon, type);
				return new ResponseEntity<>(contains, HttpStatus.OK);
			} catch (UnsupportedOperationException e) {
				return new ResponseEntity<>(http.errorHeaders(e.getMessage()), HttpStatus.INTERNAL_SERVER_ERROR);
			} catch (IllegalArgumentException e) {
