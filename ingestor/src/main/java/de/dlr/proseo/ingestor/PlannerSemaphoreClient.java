...
public class PlannerSemaphoreClient {
	...
	public Boolean acquireSemaphore(String user, String password) {
		if (logger.isTraceEnabled())
			logger.trace(">>> acquireSemaphore({}, PWD)", user);
		...
		if (!HttpStatus.OK.equals(response.getStatusCode())) {
			throw new ProcessingException(logger.log(IngestorMessage.ERROR_ACQUIRE_SEMAPHORE, response.getStatusCode().toString()));
		}
		return true;
	}
	...
	public Boolean releaseSemaphore(String user, String password) {
		if (logger.isTraceEnabled())
			logger.trace(">>> releaseSemaphore({}, PWD)", user);
		...
		ResponseEntity<String> response = restTemplate.getForEntity(url, String.class);
		if (!HttpStatus.OK.equals(response.getStatusCode())) {
			throw new ProcessingException(logger.log(IngestorMessage.ERROR_RELEASE_SEMAPHORE, response.getStatusCode().toString()));
		}
		return true;
	}
