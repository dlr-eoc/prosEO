...
public class CleanupProductThread extends Thread {
	...
	/**
	 * Start the cleanup cycle thread to look for deletable products and its files
	 * every cleanupCycleTime hours
	 *
	...
	public void run() {
		...
		while (!this.isInterrupted()) {
			if (logger.isTraceEnabled())
				logger.trace(">>> cleanup products cycle()");
