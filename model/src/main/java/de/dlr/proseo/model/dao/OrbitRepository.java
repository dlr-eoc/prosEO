...
public interface OrbitRepository extends JpaRepository<Orbit, Long> {
	 ...
	 * @return a list of orbits satisfying the spacecraft code and the orbit number
	 *         range
	 */
	@Query("select o from Orbit o where o.spacecraft.mission.code = ?1 and o.spacecraft.code = ?2 and o.orbitNumber between ?3 and ?4")
	public List<Orbit> findByMissionCodeAndSpacecraftCodeAndOrbitNumberBetween(String missionCode, String spacecraftCode,
			Integer orbitNumberFrom, Integer orbitNumberTo);

	/**
	 * Get all orbits for a given spacecraft that start in the given time interval
	 * (inclusive)
	 * 
	 * @param missionCode    the mission code
	 * @param spacecraftCode the spacecraft code
	...
	public List<Orbit> findByMissionCodeAndSpacecraftCodeAndStartTimeBetween(String missionCode, String spacecraftCode,
			Instant startTimeFrom, Instant startTimeTo);

	/**
	 * Get all orbits for a given spacecraft that start and stop time intersects the
	 * given time interval (inclusive)
	 * 
	 * @param missionCode    the mission code
	 * @param spacecraftCode the spacecraft code
