...
CREATE VIEW public.product_processing_facilities AS
 WITH RECURSIVE available_facilities(product_id, enclosing_product_id, processing_facility_id, depth, path, cycle) AS (
         SELECT p.id,
           ...
           FROM (public.product p
             LEFT JOIN public.product_file pf ON ((p.id = pf.product_id)))
        UNION
         SELECT p.id,
            p.enclosing_product_id,
                CASE
...
ALTER TABLE public.product_processing_facilities OWNER TO postgres;
...
--
-- Name: workflow uk3je18ux0wru0pxv6un40yhbn4; Type: CONSTRAINT; Schema: public; Owner: postgres
--
