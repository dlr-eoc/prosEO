			...
			for (ProductionType mpt : ProductionType.values()) {
				...
				while (timeFrom.isBefore(now)) {
					...
					if (mppdList.isEmpty()) {
						...
						// should not be
						// create a warning
						logger.log(MonitorMessage.DUPLICATE_ENTRIES, "MonProductProductionHour", timeFrom);
						mppd = mppdList.get(0);
					} else {
						mppd = mppdList.get(0);
					}
			...
			for (ProductionType mpt : ProductionType.values()) {
				...
				while (timeFrom.isBefore(now)) {
					...
					if (mppdList.isEmpty()) {
						...
						// should not be
						// create a warning
						logger.log(MonitorMessage.DUPLICATE_ENTRIES, "MonProductProductionDay", timeFrom);
						mppd = mppdList.get(0);
					} else {
						mppd = mppdList.get(0);
					}
