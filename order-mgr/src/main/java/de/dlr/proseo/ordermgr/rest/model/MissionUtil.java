...
public class MissionUtil {
	...
	public static RestMission toRestMission(Mission modelMission) {
		if (logger.isTraceEnabled())
			logger.trace(">>> toRestMission({})", (null == modelMission ? "MISSING" : modelMission.getId()));
		...
		if (null != modelMission.getSpacecrafts()) {
			List<RestSpacecraft> restSpacecrafts = new ArrayList<>();
	...
	public static Mission toModelMission(RestMission restMission) throws IllegalArgumentException {
		...
		if (logger.isTraceEnabled())
			logger.trace(">>> toModelMission({})", (null == restMission ? "MISSING" : restMission.getId()));
