...
public class ProcessorManager {
	...
	public RestProcessor createProcessor(RestProcessor processor) throws IllegalArgumentException, SecurityException {
		if (logger.isTraceEnabled())
			logger.trace(">>> createProcessor({})", (null == processor ? "MISSING" : processor.getProcessorName()));
		...
		if (!securityService.isAuthorizedForMission(processor.getMissionCode())) {
			throw new SecurityException(logger.log(GeneralMessage.ILLEGAL_CROSS_MISSION_ACCESS, processor.getMissionCode(),
		...
		if (null != RepositoryService.getProcessorRepository()
			.findByMissionCodeAndProcessorNameAndProcessorVersion(processor.getMissionCode(), processor.getProcessorName(),
					processor.getProcessorVersion())) {
			throw new IllegalArgumentException(logger.log(ProcessorMgrMessage.DUPLICATE_PROCESSOR, processor.getMissionCode(),
					processor.getProcessorName(), processor.getProcessorVersion()));
		}
		...
		if (null == modelProcessor.getProcessorClass()) {
			throw new IllegalArgumentException(logger.log(ProcessorMgrMessage.PROCESSOR_CLASS_INVALID, processor.getProcessorName(),
		...
		for (RestTask task : processor.getTasks()) {
			...
			if (modelTask.getIsCritical() && null == modelTask.getCriticalityLevel()) {
				throw new IllegalArgumentException(
						logger.log(ProcessorMgrMessage.CRITICALITY_LEVEL_MISSING, modelTask.getTaskName()));
			}
			modelTask.setProcessor(modelProcessor);
			modelTask = RepositoryService.getTaskRepository().save(modelTask);
		}

		logger.log(ProcessorMgrMessage.PROCESSOR_CREATED, modelProcessor.getProcessorClass().getProcessorName(),
				modelProcessor.getProcessorVersion(), modelProcessor.getProcessorClass().getMission().getCode());
	...
	public List<RestProcessor> getProcessors(String mission, String processorName, String processorVersion, Integer recordFrom,
		...
		if (result.isEmpty()) {
			throw new NoResultException(
					logger.log(ProcessorMgrMessage.PROCESSOR_NOT_FOUND, mission, processorName, processorVersion));
		}
	...
	public RestProcessor getProcessorById(Long id) throws IllegalArgumentException, NoResultException, SecurityException {
		if (logger.isTraceEnabled())
			logger.trace(">>> getProcessorById({})", id);
	 ...
	 */
	public RestProcessor modifyProcessor(Long id, @Valid RestProcessor processor)
			throws EntityNotFoundException, IllegalArgumentException, SecurityException, ConcurrentModificationException {
		if (logger.isTraceEnabled())
			logger.trace(">>> modifyProcessor({}, {})", id, (null == processor ? "MISSING" : processor.getProcessorName()));
		...
		if (null == processor.getProcessorVersion() || processor.getProcessorVersion().isBlank()) {
			throw new IllegalArgumentException(
					logger.log(GeneralMessage.FIELD_NOT_SET, "processorVersion", "processor modification"));
		}
		if (null == processor.getTasks() || processor.getTasks().isEmpty()) {
		...
		if (logger.isTraceEnabled())
			logger.trace("... scalar attributes for processor have changed: " + processorChanged);
		...
		for (int i = 0; i < processor.getTasks().size(); ++i) {
			...
			if (modelProcessor.getTasks().size() <= i) {
				...
				if (!modelTask.getTaskName().equals(changedTask.getTaskName())) {
					if (logger.isTraceEnabled())
						logger.trace(String.format("... task name changed from %s to %s", modelTask.getTaskName(),
				...
				if (!modelTask.getTaskVersion().equals(changedTask.getTaskVersion())) {
					if (logger.isTraceEnabled())
						logger.trace(String.format("... task version changed from %s to %s", modelTask.getTaskVersion(),
								changedTask.getTaskVersion()));
					processorChanged = true;
				...
				if (!modelTask.getIsCritical().equals(changedTask.getIsCritical())) {
					if (logger.isTraceEnabled())
						logger.trace(String.format("... task criticality changed from %s to %s",
								modelTask.getIsCritical().toString(), changedTask.getIsCritical().toString()));
					processorChanged = true;
				...
				if (!Objects.equals(modelTask.getCriticalityLevel(), changedTask.getCriticalityLevel())) {
					if (logger.isTraceEnabled())
						logger.trace(String.format("... task criticality level changed from %d to %d",
								modelTask.getCriticalityLevel(), changedTask.getCriticalityLevel()));
					processorChanged = true;
				...
				if (!Objects.equals(modelTask.getNumberOfCpus(), changedTask.getNumberOfCpus())) {
					if (logger.isTraceEnabled())
						logger.trace(String.format("... task number of cpus changed from %s to %s",
								String.valueOf(modelTask.getNumberOfCpus()), String.valueOf(changedTask.getNumberOfCpus())));
					processorChanged = true;
					modelTask.setNumberOfCpus(changedTask.getNumberOfCpus());
				}
				if (!Objects.equals(modelTask.getMinMemory(), changedTask.getMinMemory())) {
					if (logger.isTraceEnabled())
						logger.trace(String.format("... task minimum memory changed from %s to %s",
								String.valueOf(modelTask.getMinMemory()), String.valueOf(changedTask.getMinMemory())));
					processorChanged = true;
				...
				if (!modelTask.getBreakpointFileNames().equals(changedTask.getBreakpointFileNames())) {
					if (modelTask.getBreakpointFileNames().isEmpty() && changedTask.getBreakpointFileNames().isEmpty()) {
						if (logger.isTraceEnabled())
							logger.trace("... task breakpoint files 'not equal', although both are empty");
					} else {
						if (logger.isTraceEnabled())
							logger.trace(String.format("... task breakpoint files changed from %s to %s",
									modelTask.getBreakpointFileNames().toString(),
									changedTask.getBreakpointFileNames().toString()));
						processorChanged = true;
						modelTask.setBreakpointFileNames(changedTask.getBreakpointFileNames());
					}
				}
				if (logger.isTraceEnabled())
					logger.trace("... processor has changed after task " + i + ": " + processorChanged);
			}
	...
	public void deleteProcessorById(Long id)
		...
		if (logger.isTraceEnabled())
			logger.trace(">>> deleteProcessorById({})", id);
		...
		if (!modelProcessor.get().getConfiguredProcessors().isEmpty()) {
			throw new IllegalArgumentException(logger.log(ProcessorMgrMessage.PROCESSOR_HAS_CONFIG,
					...
					modelProcessor.get().getProcessorClass().getProcessorName(), modelProcessor.get().getProcessorVersion()));
		}
	...
	private Query createProcossorsQuery(String mission, String processorName, String processorVersion, Integer recordFrom,
		...
		if (logger.isTraceEnabled())
			logger.trace(">>> createProcossorsQuery({}, {}, {}, {}, {}, {})", mission, processorName, recordFrom, recordTo,
					orderBy, count);
