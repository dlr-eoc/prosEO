...
public class ConfigurationUtil {
	...
	public static RestConfiguration toRestConfiguration(Configuration modelConfiguration) {
		if (logger.isTraceEnabled())
			logger.trace(">>> toRestConfiguration({})", (null == modelConfiguration ? "MISSING" : modelConfiguration.getId()));
		...
		for (String paramKey : modelConfiguration.getDynProcParameters().keySet()) {
			restConfiguration.getDynProcParameters()
		...
		for (ConfigurationFile configurationFile : modelConfiguration.getConfigurationFiles()) {
			restConfiguration.getConfigurationFiles()
				.add(new RestConfigurationFile(configurationFile.getFileVersion(), configurationFile.getFileName()));
		}

		for (ConfigurationInputFile staticInputFile : modelConfiguration.getStaticInputFiles()) {
			restConfiguration.getStaticInputFiles()
				.add(new RestConfigurationInputFile(staticInputFile.getId(), Long.valueOf(staticInputFile.getVersion()),
						staticInputFile.getFileType(), staticInputFile.getFileNameType(),
						new ArrayList<>(staticInputFile.getFileNames())));
		}

		for (Entry<String, String> dockerRunParam : modelConfiguration.getDockerRunParameters().entrySet()) {
			restConfiguration.getDockerRunParameters()
				.add(new RestStringParameter(dockerRunParam.getKey(), dockerRunParam.getValue()));
		}
	...
	public static Configuration toModelConfiguration(RestConfiguration restConfiguration) throws IllegalArgumentException {
		...
		for (RestParameter restDynProcParam : restConfiguration.getDynProcParameters()) {
			...
			modelDynProcParam.init(ParameterType.valueOf(restDynProcParam.getParameterType()),
					restDynProcParam.getParameterValue());
