...
public class ProcessorClassUtil {
	...
	public static RestProcessorClass toRestProcessorClass(ProcessorClass modelProcessorClass) {
		if (logger.isTraceEnabled())
			logger.trace(">>> toRestProcessorClass({})", (null == modelProcessorClass ? "MISSING" : modelProcessorClass.getId()));
	...
	/**
	 * Convert a REST processor class into a prosEO model processor class (scalar and embedded attributes only, no object
	 * references)
	 *
	...
	public static ProcessorClass toModelProcessorClass(RestProcessorClass restProcessorClass) throws IllegalArgumentException {
		if (logger.isTraceEnabled())
			logger.trace(">>> toModelProcessorClass({})",
					(null == restProcessorClass ? "MISSING" : restProcessorClass.getProcessorName()));
