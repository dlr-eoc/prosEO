...
public class SampleWrapper extends BaseWrapper {
	...
	/**
	 * Sample for mission-specific modifications to the job order document before
	 * fetching input data
	 *
	...
	protected void preFetchInputHook(JobOrder jobOrderDoc) throws WrapperException {
		if (logger.isTraceEnabled())
			logger.trace(">>> preFetchInputHook({})", jobOrderDoc.getFileName());

		// Mission-specific modifications go here ...
	}

	/**
	 * Sample for mission-specific modifications to the job order document after
	 * fetching input data
	 *
	 * @param jobOrderDoc the job order document to modify
	 * @throws WrapperException if some error occurred which forces wrapper
	...
	protected void postFetchInputHook(JobOrder jobOrderDoc) throws WrapperException {
		if (logger.isTraceEnabled())
			logger.trace(">>> postFetchInputHook({})", jobOrderDoc.getFileName());
	...
	/**
	 * Sample for mission-specific modifications to the final job order document
	 * after execution of the processor (before push of results).
	 *
	...
	protected void postProcessingHook(JobOrder joWork) throws WrapperException {
		if (logger.isTraceEnabled())
			logger.trace(">>> postProcessingHook({})", joWork.getFileName());
