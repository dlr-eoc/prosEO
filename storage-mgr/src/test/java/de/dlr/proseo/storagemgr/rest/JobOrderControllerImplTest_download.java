...
public class JobOrderControllerImplTest_download {
	...
	private void downloadRestJobOrder() throws Exception {
		...
		MvcResult mvcUploadResult = mockMvc.perform(uploadRequest).andExpect(status().is(201)).andReturn();
		
		// show path of created rest job
		String json = mvcUploadResult.getResponse().getContentAsString();
		...
		MockHttpServletRequestBuilder downloadRequest = MockMvcRequestBuilders.get(REQUEST_STRING).param("pathInfo", pathInfo);

		MvcResult mvcDownloadResult = mockMvc.perform(downloadRequest).andExpect(status().isOk()).andReturn();

		System.out.println("REQUEST: " + REQUEST_STRING);
		System.out.println("Status: " + mvcDownloadResult.getResponse().getStatus());
		System.out.println("Content: " + mvcDownloadResult.getResponse().getContentAsString());
		
		// show path of created rest job
		System.out.println("Downloaded job order content: " + mvcDownloadResult.getResponse().getContentAsString());
	}
}
