...
public class LoginManager {
	 ...
	 * @return a list of strings denoting authorities granted to the user for the given mission (may be empty, meaning access is
	 *         denied)
	 */
	private List<String> login(String username, String password, String mission) {
		if (logger.isTraceEnabled())
			logger.trace(">>> login({}, ********, {})", username, mission);
		...
		try {
			for (Object authority : backendConnector.getFromService(backendConfig.getUserManagerUrl(),
					"/login" + (null == mission ? "" : "?mission=" + mission), List.class, username, password)) {
			...
			System.err.println(ProseoLogger.format(UIMessage.HTTP_CONNECTION_FAILURE, e.getMessage()));
		}
	 ...
	 * Log in to prosEO
	 *
	 * @param username         the user name for login (without mission prefix; optional, will be requested from standard input, if
	 *                         not set)
	...
	public boolean doLogin(String username, String password, String mission, boolean showLoginMessage) {
		if (logger.isTraceEnabled())
			logger.trace(">>> doLogin({}, ********, {})", username, mission);
		...
		if (null == username || username.isBlank()) {
			...
			String message = "(E" + UIMessage.LOGIN_CANCELLED.getCode() + ") " + UIMessage.LOGIN_CANCELLED.getMessage();
		...
		if (grantedAuthorities.isEmpty()) {
			...
			if (logger.isDebugEnabled())
				logger.debug("... with authorities: " + Arrays.toString(grantedAuthorities.toArray()));
			if (showLoginMessage) {
	 ...
	 *
	 * @return the mission prefix or an empty string, if no user is logged in, or if the current user is not logged in to any
	 *         mission
	 */
	...
	public boolean isPasswordStrengthOk(String password) {
		...
		if (password.matches(".*\\p{Digit}.*"))
			++elementGroupCount;
		if (password.matches(".*[" + passwordSpecialChars + "].*"))
			++elementGroupCount;
		ok = ok && passwordMinElements <= elementGroupCount;

		// Log and (in interactive mode) print error message, if strength check failed
		if (!ok) {
			String message = logger.log(UIMessage.PASSWORD_STRENGTH_INSUFFICIENT, passwordMinLength, passwordMinElements,
